...
Requires **Ubuntu 22.04+** or an equivalent distro with `libwebkit2gtk-4.1-dev`. Linux builds use xcap 0.4.x (X11/dbus) so PipeWire 1.0+ is not required. `npm run setup:install` installs Tauri and screen-capture build dependencies. See [Tauri prerequisites](https://v2.tauri.app/start/prerequisites/) for other distributions.
